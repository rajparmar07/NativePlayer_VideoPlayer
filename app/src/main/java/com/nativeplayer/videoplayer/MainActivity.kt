...
import java.io.File
import com.nativeplayer.videoplayer.viewmodel.ListDisplayMode
...
import com.nativeplayer.videoplayer.ui.screens.*
import com.nativeplayer.videoplayer.ui.theme.MyApplicationTheme
import com.nativeplayer.videoplayer.viewmodel.VideoPlayerViewModel
import com.nativeplayer.videoplayer.viewmodel.VideoPlayerViewModelFactory
...
class MainActivity : ComponentActivity() {
    ...
    private val pipReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            intent?.action?.let { action ->
                when (action) {
                    "com.nativeplayer.videoplayer.ACTION_PLAY_PAUSE" -> {
                    ...
                    "com.nativeplayer.videoplayer.ACTION_NEXT" -> {
                        viewModel.playNext()
                    }
                    "com.nativeplayer.videoplayer.ACTION_PREV" -> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = IntentFilter().apply {
            addAction("com.nativeplayer.videoplayer.ACTION_PLAY_PAUSE")
            addAction("com.nativeplayer.videoplayer.ACTION_NEXT")
            addAction("com.nativeplayer.videoplayer.ACTION_PREV")
        }
        ...
        setContent {
            ...
            MyApplicationTheme(
                ...
                var isPlaylistsSettingsOpen by remember { mutableStateOf(false) }
                var isAboutOpen by remember { mutableStateOf(false) }
                var isOpenSourceLibrariesOpen by remember { mutableStateOf(false) }

                val selectedPlaylist by viewModel.selectedPlaylist.collectAsState()

                // Back: Open Source Libraries sub-sub-screen (highest priority)
                BackHandler(enabled = isOpenSourceLibrariesOpen && currentPlayingVideo == null) {
                    isOpenSourceLibrariesOpen = false
                }

                // Back: About sub-screen
                BackHandler(enabled = isAboutOpen && !isOpenSourceLibrariesOpen && currentPlayingVideo == null) {
                    isAboutOpen = false
                }

                // Back: Appearance settings sub-screen
                ...
                // Back: Playlists settings sub-screen
                BackHandler(enabled = isPlaylistsSettingsOpen && currentPlayingVideo == null) {
                    isPlaylistsSettingsOpen = false
                }

                // Back: Settings screen
                BackHandler(enabled = isSettingsOpen && !isAppearanceSettingsOpen && !isAudioSettingsOpen && !isMemoryOpen && !isPlaybackSettingsOpen && !isGesturesSettingsOpen && !isPlaylistsSettingsOpen && !isAboutOpen && !isOpenSourceLibrariesOpen && currentPlayingVideo == null) {
                    isSettingsOpen = false
                }

                val isSettingsActive = (isSettingsOpen || isAppearanceSettingsOpen || isAudioSettingsOpen || isPlaybackSettingsOpen || isGesturesSettingsOpen || isPlaylistsSettingsOpen || isMemoryOpen || isAboutOpen || isOpenSourceLibrariesOpen) && currentPlayingVideo == null

                // Back: Selected Playlist detail view
                BackHandler(enabled = currentTab == ScreenTab.Playlists && selectedPlaylist != null && !isSettingsActive && currentPlayingVideo == null) {
                ...
                BackHandler(enabled = !isSettingsOpen && !isAppearanceSettingsOpen && !isAudioSettingsOpen && !isMemoryOpen && !isPlaybackSettingsOpen && !isGesturesSettingsOpen && !isPlaylistsSettingsOpen && !isAboutOpen && !isOpenSourceLibrariesOpen && !(currentTab == ScreenTab.Playlists && selectedPlaylist != null) && currentPlayingVideo == null) {
                ...
                Box(modifier = Modifier.fillMaxSize()) {
                    ...
                    Scaffold(
                        ...
                        topBar = {
                            ...
                            val playlistItems by if (selectedPlaylist != null) viewModel.getPlaylistItems(selectedPlaylist!!.id).collectAsState(initial = emptyList()) else remember { mutableStateOf(emptyList()) }
                            val playlistTitle = if (selectedPlaylist != null) selectedPlaylist!!.title else "Playlists"
                            val playlistSubtitle = if (selectedPlaylist != null) {
                                if (playlistItems.size == 1) "1 video in playlist" else "${playlistItems.size} videos in playlist"
                            } else {
                            ...
                            StreamCacheTopBar(
                                ...
                                navigationIcon = when (currentTab) {
                                    ScreenTab.Local -> {
                                        if (isSearchingMode) {
                                            ...
                                            {
                                                IconButton(
                                                    onClick = { viewModel.clearSelectedVideoIds() },
                                ...
                                actions = {
                                    // Select All / Deselect All Button (visible ONLY when selecting)
                                    AnimatedVisibility(
                                        ...
                                        IconButton(
                                            onClick = {
                                                viewModel.setSelectedVideoIds(if (allSelected) emptySet() else currentList.map { it.id }.toSet())
                                            },
                                            modifier = Modifier
                                    ...
                                    // New Playlist Button (visible ONLY on Playlists tab when NOT inside a playlist)
                                    AnimatedVisibility(
                                        visible = currentTab == ScreenTab.Playlists && selectedPlaylist == null,
                                        enter = expandHorizontally(expandFrom = Alignment.End, animationSpec = tween(220, easing = LinearOutSlowInEasing)) + fadeIn(tween(200)) + scaleIn(initialScale = 0.85f),
                                        exit = shrinkHorizontally(shrinkTowards = Alignment.End, animationSpec = tween(220, easing = LinearOutSlowInEasing)) + fadeOut(tween(180)) + scaleOut(targetScale = 0.85f)
                                    ) {
                                        IconButton(
                                    ...
                                    // Settings Button — hidden when in selection mode
                                    AnimatedVisibility(
                                        visible = !(currentTab == ScreenTab.Local && selectedVideoIds.isNotEmpty()),
                                        enter = expandHorizontally(expandFrom = Alignment.End, animationSpec = tween(220, easing = LinearOutSlowInEasing)) + fadeIn(tween(200)) + scaleIn(initialScale = 0.85f),
                                        exit = shrinkHorizontally(shrinkTowards = Alignment.End, animationSpec = tween(220, easing = LinearOutSlowInEasing)) + fadeOut(tween(180)) + scaleOut(targetScale = 0.85f)
                        ...
                        bottomBar = {
                            ...
                            val isSearchingMode by viewModel.isSearchingMode.collectAsState()
                            val selectedVideoIds by viewModel.selectedVideoIds.collectAsState()
                            val isVideoListScreen = currentTab == ScreenTab.Local && (
                                selectedFolder != null ||
                                selectedTreePath != null ||
                                isSearchingMode ||
                                selectedVideoIds.isNotEmpty()
                            )
                            val isSpecificPlaylistScreen = currentTab == ScreenTab.Playlists && selectedPlaylist != null

                            AnimatedVisibility(
                                visible = !isVideoListScreen && !isSpecificPlaylistScreen,
                    ...
                    AnimatedVisibility(
                        ...
                        val activeSubScreen = remember(isAppearanceSettingsOpen, isAudioSettingsOpen, isPlaybackSettingsOpen, isGesturesSettingsOpen, isPlaylistsSettingsOpen, isMemoryOpen, isAboutOpen, isOpenSourceLibrariesOpen) {
                            when {
                                isOpenSourceLibrariesOpen -> "Open Source Libraries"
                                isAboutOpen               -> "About"
                                ...
                                isPlaylistsSettingsOpen   -> "Playlists"
                        ...
                        Surface(
                            ...
                            Column(modifier = Modifier.fillMaxSize()) {
                                ...
                                StreamCacheTopBar(
                                    ...
                                    navigationIcon = {
                                        IconButton(
                                            onClick = {
                                                when {
                                                    isOpenSourceLibrariesOpen -> isOpenSourceLibrariesOpen = false
                                                    ...
                                                    isPlaylistsSettingsOpen  -> isPlaylistsSettingsOpen = false
                                ...
                                Box(
                                    ...
                                    val getScreenDepth = { screenName: String ->
                                        when (screenName) {
                                            "Settings" -> 0
                                    ...
                                    AnimatedContent(
                                        ...
                                        transitionSpec = {
                                            val isForward = getScreenDepth(targetState) > getScreenDepth(initialState)
                                            if (isForward) {
                                                (slideInHorizontally(animationSpec = tween(300), initialOffsetX = { it }) + fadeIn(animationSpec = tween(300)))
                                        ...
                                        when (currentSubScreen) {
                                            "Settings" -> {
                                                SettingsScreen(
                                                    ...
                                                    onNavigateToPlaylists = { isPlaylistsSettingsOpen = true },
                                                    onNavigateToMemory = { isMemoryOpen = true },
                                                    onNavigateToAbout = { isAboutOpen = true },
                                                    includeTopBar = false
                                                )
                                            }
                                            "Appearance" -> {
                                            ...
                                            "Playlists" -> {
                                                PlaylistsSettingsScreen(
                                                    viewModel = viewModel,
                                                    onBack = { isPlaylistsSettingsOpen = false },
                                                    includeTopBar = false
                                                )
                                            }
                                            "Memory & Cache" -> {
                                            ...
                                            "About" -> {
                                                AboutScreen(
                                                    ...
                                                    onNavigateToOpenSourceLibraries = { isOpenSourceLibrariesOpen = true },
                                            ...
                                            "Open Source Libraries" -> {
                                                OpenSourceLibrariesScreen(
                                                    onBack = { isOpenSourceLibrariesOpen = false },
                                                    onShowSnackbar = { msg -> viewModel.showSnackbar(msg) },
                                                    includeTopBar = false
                                                )
                                            }
                    ...
                    // Animated Splash Screen Overlay (Plays GIF on matching black canvas, auto-transitions)
                    AnimatedVisibility(
                        visible = showSplash,
    ...
    private fun updatePipParams(isPlaying: Boolean) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            // Prev Action
            val prevIntent = Intent("com.nativeplayer.videoplayer.ACTION_PREV")
            ...
            // Play/Pause Action
            val playPauseIntent = Intent("com.nativeplayer.videoplayer.ACTION_PLAY_PAUSE")
            ...
            // Next Action
            val nextIntent = Intent("com.nativeplayer.videoplayer.ACTION_NEXT")
